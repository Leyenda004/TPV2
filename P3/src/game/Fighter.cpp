...
void Fighter::update() {
	...
	//Game::Instance()->get_networking().send_state(p.pos, p.width, p.height, p.rot);
...
void Fighter::shoot(Player &p) {
	if (_ihdlr.keyDownEvent() && _ihdlr.isKeyDown(SDL_SCANCODE_SPACE)) {
		...
		//Game::Instance()->get_networking().send_shoot(bp, bv, bwidth, bheight, Vector2D(0, -1).angle(bv));
...
void Fighter::send_my_info() {
	...
	//Game::Instance()->get_networking().send_my_info(p.pos, p.width, p.height, p.rot, p.state);
}
